        ...
        <div className="grid gap-8">
          {users.map((user) => (
            <div key={user.id} className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
              ...
              {/* --- NEW: Custom Messages & Account Info Form --- */}
              <form action={updateUserCustomizations} className="mb-8 bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-4 shadow-inner">
                <div className="flex justify-between items-center border-b border-slate-900 pb-2 mb-4">
                  ...
                  <button className="text-[10px] bg-slate-800 hover:bg-slate-700 text-white px-3 py-1 rounded transition-colors">Save Updates</button>
                </div>
                <input type="hidden" name="userId" value={user.id} />

                <div>
                  <label className="text-[9px] text-slate-600 block uppercase mb-1">Account Number</label>
                  <input name="accountNumber" type="text" defaultValue={user.accountNumber || ""} placeholder="e.g. 1029384756" className="w-full bg-transparent text-white text-sm outline-none border-b border-slate-800 focus:border-emerald-500 py-1" />
                </div>

                <div>
                  <label className="text-[9px] text-slate-600 block uppercase mb-1">Vault Status Message (Dashboard Note)</label>
                  <textarea name="vaultStatusMessage" defaultValue={user.vaultStatusMessage || ""} placeholder="Note: Your account is in Safe Mode..." className="w-full bg-slate-900/50 text-white text-xs outline-none border border-slate-800 focus:border-emerald-500 rounded-lg p-3 h-16 resize-none" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[9px] text-slate-600 block uppercase mb-1">Send Modal Pop-up</label>
                    <textarea name="sendMessage" defaultValue={user.sendMessage || ""} placeholder="This account is marked Dormant..." className="w-full bg-slate-900/50 text-white text-xs outline-none border border-slate-800 focus:border-emerald-500 rounded-lg p-3 h-20 resize-none" />
                  </div>
                  <div>
                    <label className="text-[9px] text-slate-600 block uppercase mb-1">Receive/Deposit Pop-up Note</label>
                    <textarea name="receiveMessage" defaultValue={user.receiveMessage || ""} placeholder="Custom deposit instructions..." className="w-full bg-slate-900/50 text-white text-xs outline-none border border-slate-800 focus:border-emerald-500 rounded-lg p-3 h-20 resize-none" />
                  </div>
                </div>
              </form>
